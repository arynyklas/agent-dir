...
Once the prototype has answered its question, capture the answer the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set informs the real module (the decision, absorbed through the normal production workflow); the HTML shell is deleted unless the user asks to preserve it as a local artifact, branch, or commit.
